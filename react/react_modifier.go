...
// Package react allows for the replacement of message body on responses.
package react
...
func init() {
	parse.Register("react.Modifier", modifierFromHTML)
}
...
func NewModifier(b []byte) *Modifier {
	log.Debugf("react.NewModifier: len(b): %d, contentType %s", len(b))
	return &Modifier{
		body:     b,
...
// {
//   "scope": ["response"],
//   "url": "" // Base64 encoded body
// }
func modifierFromHTML(b []byte) (*parse.Result, error) {
	msg := &modifierHMTL{}
	if err := json.Unmarshal(b, msg); err != nil {
		return nil, err
	}

	mod := NewModifier(msg.Body)
	return parse.NewResult(mod, msg.Scope)
}

// ModifyRequest sets the Content-Type header and overrides the request body.
func (m *Modifier) ModifyRequest(req *http.Request) error {
	log.Debugf("react.ModifyRequest: request: %s", req.URL)
	return nil
}
...
func (m *Modifier) ModifyResponse(res *http.Response) error {
	...
	if res.Request.Header.Get("Range") == "" {
		...
		res.Body = ioutil.NopCloser(bytes.NewReader([]byte("oulalal")))
